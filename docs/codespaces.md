...
Per [`qte77/dotfiles`'s own README](https://github.com/qte77/dotfiles#codespaces-dotfiles-behavior), dotfiles installation "only applies to **new** codespaces, not existing ones" — so `install.sh` does **not** re-run on a rebuild of an *existing* codespace, and can't re-create the symlink there. `setup_claude_home` (run first in `setup_all`, via vendored `scripts/link-claude-home.sh`) is what actually re-links it on every `onCreateCommand` of an existing codespace's rebuild — not a defense-in-depth backstop, the load-bearing mechanism for that case. Dotfiles' `install.sh` remains the mechanism for a *brand-new* codespace's first creation.
